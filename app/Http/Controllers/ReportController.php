    ...
    public function attendance_report_index(Request $request)
    {
        ...
        if ($user_auth->can('attendance_report')) {
            ...
            if ($request->ajax()) {
                ...
                foreach ($attendances_Filtred as $attendance) {
                    if ($attendance->date) {
                        $attendance->date =  $attendance->date ? \Carbon\Carbon::parse($attendance->date)->format('d/m/Y') : '';
                    }
                }
    ...
    public function project_report_index(Request $request)
    {
        ...
        if ($user_auth->can('project_report')) {
            ...
            if ($request->ajax()) {
                ...
                foreach ($projects_Filtred as $project) {
                    if ($project->start_date) {
                        $project->start_date =  $project->start_date ? \Carbon\Carbon::parse($project->start_date)->format('d/m/Y') : '';
                    }
                    if ($project->end_date) {
                        $project->end_date =  $project->end_date ? \Carbon\Carbon::parse($project->end_date)->format('d/m/Y') : '';
                    }
                }
    ...
    public function task_report_index(Request $request)
    {
        ...
        if ($user_auth->can('task_report')) {
            ...
            if ($request->ajax()) {
                ...
                foreach ($tasks_Filtred as $task) {
                    if ($task->start_date) {
                        $task->start_date =  $task->start_date ? \Carbon\Carbon::parse($task->start_date)->format('d/m/Y') : '';
                    }

                    if ($task->end_date) {
                        $task->end_date =  $task->end_date ? \Carbon\Carbon::parse($task->end_date)->format('d/m/Y') : '';
                    }
                }
    ...
    public function deposit_report_index(Request $request)
    {
        ...
        if ($user_auth->can('deposit_report')) {
            ...
            if ($request->ajax()) {
                ...
                foreach ($deposits_Filtred as $deposit) {
                    if ($deposit->date) {
                        $deposit->date =  $deposit->date ? \Carbon\Carbon::parse($deposit->date)->format('d/m/Y') : '';
                    }
                }
